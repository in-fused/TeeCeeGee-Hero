  ...
  return (
    <PageTransition>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <h1 className="text-2xl font-bold text-white mb-2">Prices &amp; Stock</h1>
        <p className="text-sm text-gray-400 mb-6">
          Live pricing from TCGplayer, eBay, and more. Search for sealed products to see current prices.
        </p>
...
function StatsPanel() {
  ...
  useEffect(() => {
    getPriceStats().then(setData).catch(() => {}).finally(() => setLoading(false));
  }, []);
      ...
      {loading ? <Spinner /> : !data ? (
        ...
        <div className="space-y-3 text-sm">
          <div className="flex justify-between text-gray-300">
            ...
            <span className="text-white font-medium">{data.activeListings}</span>
          </div>
          <div className="flex justify-between text-gray-300">
            <span>Total Listings</span>
            <span className="text-white font-medium">{data.totalListings}</span>
          </div>
          <div className="flex justify-between text-gray-300">
            <span>Recent Changes (24h)</span>
            <span className="text-white font-medium">{data.recentChanges}</span>
          </div>
            ...
            <div className="pt-2 border-t border-[var(--color-border-subtle)]">
              ...
              {data.byRetailer.map((r) => (
                <div key={r.retailer} className="flex justify-between text-gray-300">
                  <span className={`text-xs font-medium px-2 py-0.5 rounded-full capitalize ${RETAILER_COLORS[r.retailer] || 'bg-gray-800 text-gray-400'}`}>
                    {r.retailer}
                  </span>
                  <span>{r.count}</span>
                </div>
  ...
  return (
    <Card title="Recent Changes">
      {loading ? <Spinner /> : changes.length === 0 ? (
        <p className="text-gray-500 text-sm">No recent changes detected. Changes appear when prices or stock levels update.</p>
      ) : (
        <div className="space-y-2 max-h-96 overflow-y-auto">
          {changes.map((c) => {
            const content = (
